...
import kotlinx.android.synthetic.main.activity_dificuldade_item.view.*


class DificuldadeAdapter(val context: Context, val item: List<Dificuldades>, val clickListener: (Dificuldades) -> Unit)
    : RecyclerView.Adapter<DificuldadeAdapter.ViewHolder>() {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val view = LayoutInflater.from(parent.context).inflate(R.layout.activity_dificuldade_item, parent, false)
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        holder.bindView(context, item[position], clickListener)
    ...
    class ViewHolder( itemView: View) : RecyclerView.ViewHolder(itemView) {
        @RequiresApi(Build.VERSION_CODES.O)
        fun bindView(context: Context, dificuldade: Dificuldades, clickListener: (Dificuldades) -> Unit) {
